...
let campoEmailLogin = document.getElementById('inputEmailLogin');
...
let emailEValidoLogin = false;
let senhaEValidoLogin = false;


//Definindo objeto
const usuarioObjetoLogin = {
    email: "",
    password: "",
}
...
botaoAcessar.addEventListener('click', function(evento){
    ...
    if (validacaoTelaDeLogin()) {
        ...
        usuarioObjetoLogin.email = campoEmailLoginNormalizado;
        usuarioObjetoLogin.password = campoSenhaLoginNormalizado;
        console.log(usuarioObjetoLogin);

        let endPointLogin = "https://ctd-todo-api.herokuapp.com/v1/users/login";
        let loginUsarioJson = JSON.stringify(usuarioObjetoLogin);
...
function validacaoTelaDeLogin () {
    if (emailEValidoLogin&senhaEValidoLogin) {
